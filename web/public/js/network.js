...
function initSigma(config) {
    ...
    var a = new sigma(document.getElementById("sigma-canvas"))
    //.drawingProperties(drawProps).graphProperties(graphProps).mouseProperties(mouseProps);
    sigInst = a;
    a.active = !1;
    a.neighbors = {};
    a.detail = !1;
    ...
    dataReady = function(s) {//This is called as soon as data is loaded
        console.log(s.nodes)
		a.clusters = {};
        for (var n = s.nodes, i = 0, l = n.length; i < l; i++) {
            var b = n[i];
            a.graph.addNode({ id: b.id, color: b.color });
            a.clusters[b.color] || (a.clusters[b.color] = []);
            a.clusters[b.color].push(b.id);//SAH: push id not label
        }
	
		a.bind("upnodes", function (b) {
		    nodeActive(b.content[0])
		});
    ...
    if (data.indexOf("gexf")>0 || data.indexOf("xml")>0) {
        sigma.parsers.gexf("data/"+data,dataReady(a));
    }
    else {
        // sigma.parsers.json("data/"+data,dataReady(a));
        sigma.parsers.json("data/"+data,function(s) {
            dataReady(s)
        });
    }
    gexf = sigmaInst = null;
...
function configSigmaElements(config) {
    ...
    $GP.bg = "#f00"; //$(sigInst._core.domElements.bg);
    $GP.bg2 = "#00f"; //$(sigInst._core.domElements.bg2);
    var a = [],
        b,x=1;
