...
fig_nums("corrections.exploratory", "The Exploratory Matrix of all Variables Correlated")
...
```{r correlation-skills-education, fig.width = 8, fig.height = 6, echo=FALSE}
generateCorrelation("total.431", "Q436", "skills-education")
```

 - Interest (Q437) by education level (Q436)

 ```{r correlation-interests-education, fig.width = 8, fig.height = 6, echo=FALSE}
...
As *`r cf("agg.results")`* shows, no variables correlate strongly in a negative direction - such cells would be shown in
red, but these are non-existent. Instead, there are varying degrees of positive correlations. Positive correlations are particularly marked
by the purplish squares that accompany the diagonal. These suggest that specific variables co-correlate strongly with other variables that belong to the same construct.
There is also reasonable correlation between those constructs that belong to the same theme or critical issue.

One interpretation of this pattern is confirmation that the majority of statements are positively worded, such that higher numerical scores
indicates higher levels of interests, competencies, resilience or connectedness. The positive correlation then reinforces the idea
that these various capacities are, to varying degress, both internally (within a single construct and critical issue) and externally interrelated.
Broadly, participants with high levels of interests are also likely to have high competencies, resilience and connectedness, and vice versa for each of the other issues.
More substantively, it never appears that any individual variables, contructs or themes are negatively correlated; no-one with low competencies thought they had high degrees
of resilience, interests or connectedness for instance.

Further, we can assume the patterns of clustering provide general but useful indication that the survey instrument is reasonably robust.
Responses to certain questions act as more or less moderate predictors of other responses to other questions.
This suggests some redundancy in the survey instrument, and therefore opportunity to either truncate its length or to supplement it with
measurements of other critical issues or capacities.

While we have not run a full set of regression models, on a initial regression test of the first question (Q74_1), 12 out of 157 variables were multicollinear,
as the output below shows. This suggests that in spite of the strong overall pattern of co-correlation, strict multicollinearity may not be problem for the
survey instrument as it currently stands.
...
v<-vif(fit) # variance inflation factors
table(sqrt(vif(fit)) > 2) # problem?
```
