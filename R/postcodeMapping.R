...
obtainGeoAreaXML <- function(postcode, level) {
	value <- ""
	if (level == "SA4") {
...
obtainGeoArea <- function(postcode, level) {
  value <- ""
  if (level == "SA4") {
    value <- na.omit(as.character(sa4s[sa4s$X == postcode, "X.2"]))
  }
  else if (level == "SA3") {
    value <- na.omit(as.character(sa3s[sa3s$X == postcode, "X.2"]))
  }
  else if (level == "SA2") {
    value <- na.omit(as.character(sa2s[sa2s$X == postcode, "X.2"]))
  }
  else if (level == "SLA") {
    value <- na.omit(as.character(sas[sas$X == postcode, "X.2"]))
  }
  else if (level == "LGA") {
    value <- na.omit(as.character(lgas[lgas$X == postcode, "X.2"]))
  }
  return (value)
}