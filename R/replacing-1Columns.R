...
addDemographicData = function(data) {
  ...
  # 
  # geocode = geocode(data$"Location (Postcode)", source = "google")
  # lat = geocode$lat
  # lon = geocode$lon

  LocationData <- read.csv("data/LocationData.csv")
  pcs = sample(LocationData$postcode, dim(data)[1], replace=TRUE)
  lat = LocationData$lat[which(pcs %in% LocationData$postcode)]
  lon = LocationData$lon[which(pcs %in% LocationData$postcode)]
